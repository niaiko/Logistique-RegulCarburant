...
public class TypeRepConsumer {
	...
	public void ajout(ConsumerRecord<?, ?> record) {
		try {
			...
			producer.send(record.key().toString(), "ajouttyperepRes", "success");
		} catch (Exception e) {
	...
	@KafkaListener(topics = "RechercheTypeRepreq")
	public void rechercheTypeRep(ConsumerRecord<?, ?> record) {
		try {
			//TypeRepMod param = om.readValue(record.value().toString(),TypeRepMod.class);
			//List<TypeRepMod> acces = service.rechercheTypeRep()
			//resultat = om.writeValueAsString(acces);
			log.info("nbrligne: "+resultat);
     		producer.send(record.key().toString(),  "RechercheTypeRepreq", resultat);
		} catch (Exception e) {
			producer.send(record.key().toString(), "RechercheTypeRepreq", e.getMessage());
		}
	}
